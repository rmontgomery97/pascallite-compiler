...
void AssignStmt()
{
    ...
    {
        numErrors += 1;
        listingFile << endl << "Error: Line " << lineNumber - 1 << ": non-keyword identifier or \"end\" expected";
        CreateListingTrailer();
        exit(1);
    }
    PushOperand(token);
...
void EmitAdditionCode(string operand1, string operand2)
{
    ...
    if (contentsOfAReg != operand1 && contentsOfAReg != operand2) // if neither operand is in A reg, load operand2 into A-reg
    {
        contentsOfAReg = "";
...
void EmitModuloCode(string operand1, string operand2)
{
    ...
    {                                                                                           //operand 2, deassign it,
        objectFile << setw(4) << "" << setw(2) << "" << "STA " << setw(4) << contentsOfAReg;    //store the temp into memory,
        objectFile << setw(5) << "" << "deassign A-reg" << endl;                                //and change the alloc entry in the table to yes
...
void EmitMultiplicationCode(string operand1, string operand2)
{
    ...
    objectFile << setw(4) << "" << setw(2) << "" << "IMU " << setw(4) << inName1; // emit code to perform addition
    objectFile << setw(5) << "" << operand2 << " * " << operand1 << endl;
...
void Code(string oprtr, string operand1, string operand2)
{
    if (oprtr == "program")
    {
        objectFile << "STRT  NOP" << setw(10) << "" << symbolTable[0].externalName << " - Reese Montgomery - Stage 1" << endl;
    }
    else if (oprtr == "end")
    {
    ...
    //multiply inserted names are illegal
{
    ...
    while (externalName.length() != 0) //this while loop extracts each non key id from a string delimited by commas
    {
        ...
        if(isKeyword(name) && name != "true") //check if name is a reserved keyword
        {
            numErrors += 1;
